...
public class ProgramaDao  implements  MePrograma{  
    ...
    public Programa list(int id) {
        ...
        try{
             ...
             while(rs.next())
             {
                 sny.setCodprograma(rs.getInt("codpro"));
                 sny.setNomprograma(rs.getString("nompro"));
    ...
    public boolean actualizarpro(Programa pro) {
        String sql = "update programa set codpro='"+pro.getCodprograma()+"',nompro='"+pro.getNomprograma()+"', where id="+pro.getCodprograma();
            try{
            con=cn.getConnection(); 
            ps=con.prepareStatement(sql); 
            ps.executeUpdate();
